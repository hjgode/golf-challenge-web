...
<tr>
<td>
<form name="fli_auswahl" action="flight.php" method="POST" OnSubmit>
	<font size=+3>Flight <? echo "$fli - " ;?></font>
	...
	<INPUT TYPE=button NAME="4" VALUE="Wählen" onClick="document.fli_auswahl.submit()">
</form>
</td>
</tr>
</table>
<?php
   ...
   for ($i=0; $i<$num; $i++)
   {
		...
		echo "<td>$fc$na</td>";
		echo "<td>$fc$ge</td>";
		echo "<td align=center>$fc<a href='edit_both.php?ID=$id' border=0>$ne</a></td>";
		echo "<td align=center>$fc<a href='edit_both.php?ID=$id' border=0>$lo</a></td>";
		  // Tabellenzeile mit -zellen, wechselweise Grau hinterlegen
		  if (($i % 2) !=1)
			{
			echo "</font>";
